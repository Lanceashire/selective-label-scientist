...
export function App() {
  ...
  const [session, setSession] = useState<DatasetSession | null>(null); const [confirmed, setConfirmed] = useState(false);
...
function PageContent({ page, hint, session, confirmed, onNew, onImported, onConfirmed, onResume, onExperiments, onReport }: { page: Page; hint: string; session: DatasetSession | null; confirmed: boolean; onNew: () => void; onImported: (session: DatasetSession) => void; onConfirmed: () => void; onResume: (session: DatasetSession) => void; onExperiments: (session: DatasetSession) => void; onReport: (session: DatasetSession) => void }) {
  ...
  if (page === "report" && session) return <ReportViewer session={session}/>;
...
function HealthCard() { const [health, setHealth] = useState<DesktopHealth | null>(null); const [busy, setBusy] = useState(false); const check = async () => { setBusy(true); try { setHealth(await DesktopBridge.healthCheck()); } finally { setBusy(false); } }; return <article className="card"><p>科研后端</p><h3>{health ? health.backend : "待连接"}</h3><small>{health ? `数据库：${health.database} · Agent Host：${health.agent_host}` : "通过 Tauri IPC 检查本地 Sidecar"}</small><button className="primary health-button" disabled={busy} onClick={() => void check()}>{busy ? "检查中…" : "后端状态"}</button></article>; }
