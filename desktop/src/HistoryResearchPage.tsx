...
export function HistoryResearchPage({ onResume, onExperiments, onReport }: { onResume: (session: ResumedDatasetSession) => void; onExperiments: (session: ResumedDatasetSession) => void; onReport: (session_id: string) => void }) {
  ...
  const restore = async (item: HistorySession, target: "workbench" | "experiments") => {
    setBusy(item.session_id); setNotice("正在从 SQLite 快照恢复同一 Session…");
    try { const restored = await DesktopBridge.resumeSession(item.session_id); target === "workbench" ? onResume(restored) : onExperiments(restored); }
    catch { setNotice("无法恢复该 Session：原始数据集可能已移动、删除或不再可读取。 "); }
    finally { setBusy(null); }
  };
  const viewReport = (item: HistorySession) => {
    setNotice("正在直接读取该 Session 的已生成报告…");
    onReport(item.session_id);
