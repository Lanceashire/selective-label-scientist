"""Typed RPC facade. All scientific actions delegate to ResearchRuntime only."""
...
def dispatch(action: str, payload: dict[str, Any]) -> dict[str, Any]:
    # Metrics injection is a protocol violation before any runtime code runs.
    ...
    root = payload.get("state_dir") or (Path.home() / ".ecomic")
...
def main() -> int:
    for line in sys.stdin:
        try:
            request = json.loads(line); response = dispatch(str(request.get("action", "")), dict(request.get("payload", {})))
