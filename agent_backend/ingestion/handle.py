...
class DatasetHandle:
    ...
    def open(cls, path: str | Path) -> "DatasetHandle":
        ...
        reader = (f"read_csv_auto('{escaped}', sample_size=20000, ignore_errors=false)"
                  if suffix == ".csv" else f"read_parquet('{escaped}')")
        try:
            relation = connection.sql(f"SELECT * FROM {reader}")
            columns = list(relation.columns)
            row_count = int(connection.sql(f"SELECT count(*) FROM {reader}").fetchone()[0])
        except duckdb.Error as exc:
            raise ValueError(f"dataset read failed: {exc}") from exc
        if not columns:
            raise ValueError("dataset has no columns")
        if len(set(columns)) != len(columns):
            raise ValueError("duplicate column names")
        return cls(source, streaming_sha256(source), suffix.lstrip("."), row_count,
                   columns, source.stat().st_size, connection)

    def _reader(self) -> str:
        escaped = str(self.path).replace("'", "''")
        return (f"read_csv_auto('{escaped}', sample_size=20000, ignore_errors=false)"
                if self.format == "csv" else f"read_parquet('{escaped}')")

    def query(self, sql: str) -> list[tuple[Any, ...]]:
        return self.connection.sql(sql).fetchall()

    def sample(self, limit: int = MAX_SAMPLE_ROWS_FOR_LLM) -> list[dict[str, Any]]:
        safe_limit = min(max(0, int(limit)), MAX_SAMPLE_ROWS_FOR_LLM)
        cursor = self.connection.execute(f"SELECT * FROM {self._reader()} USING SAMPLE {safe_limit} ROWS")
    ...
    def profile(self) -> dict[str, Any]:
        ...
        for column in self.columns:
            name = quote(column)
            row = self.connection.sql(f"SELECT typeof({name}), count(*) - count({name}), count(DISTINCT {name}) FROM {self._reader()}").fetchone()
            ...
            columns[column] = {"dtype": str(row[0]), "missing_count": int(row[1]), "missing_rate": round(int(row[1]) / max(1, self.row_count), 6), "unique_count": int(row[2]), "top_values": {str(v): int(n) for v, n in values}}
