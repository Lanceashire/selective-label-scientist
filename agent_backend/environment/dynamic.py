...
The policy receives only :class:`ResearchDataView`; labels are revealed only for
chosen records after a batch is consumed.  The oracle partition stays private
until ``finalize`` and is never put in an agent observation.
"""
...
def _number(value: Any) -> float:
    try:
        ...
        return float(abs(hash(str(value))) % 10_000) / 10_000
...
class DynamicSelectiveLabelEnvironment:
    def __init__(self, rows: list[dict[str, Any]], spec: dict[str, Any], *, oracle_fraction: float = .2, seed: int = 0):
        ...
        if not self.feature_columns:
            raise ProtocolViolation("no feature columns available for dynamic model")
        ...
        self._oracle = OracleEvaluationStore(self.universe.label_by_id, frozenset(self.universe.oracle_ids))
        self._fit()
    ...
    def _fit(self) -> None:
        if self._revealed:
            ids = sorted(self._revealed)
            self._model.fit(self._matrix(ids), np.asarray([self._revealed[idx] for idx in ids], dtype=int))

    def reset(self, *, total_budget: float) -> dict[str, Any]:
        if total_budget < 0:
            raise ValueError("budget must be non-negative")
        self._remaining_budget = float(total_budget)
        return self.observe_state()

    def observe_state(self) -> dict[str, Any]:
        """Safe agent-visible status; specifically excludes all hidden/oracle labels."""
        return {"round_index": self._round_index, "remaining_budget": self._remaining_budget,
                "visible_label_count": len(self._revealed), "candidate_remaining": len(self.universe.candidate_ids - self.universe.departed_candidate_ids),
                "oracle_count": len(self.universe.oracle_ids), "finished": self._finished}
    ...
    def advance_round(self, *, batch_size: int, policy: str, seed: int = 0) -> dict[str, Any]:
        if self._finished:
            raise ProtocolViolation("research is finished; no further adaptive round is allowed")
        batch = self.universe.activate_batch(batch_size)
        if not batch:
            self._finished = True
            return {"status": "EXHAUSTED", **self.observe_state()}
        view = self._view(batch)
        result: PolicyResult = execute_policy(view, policy, seed=seed)
        selected = self.universe.consume_batch(result.selected_ids)
        revealed = {idx: self.universe.label_by_id[idx] for idx in selected}
        self._revealed.update(revealed)
        self._remaining_budget = max(0.0, self._remaining_budget - result.predicted_cost)
        self.records.append(RoundRecord(self._round_index, sorted(batch), sorted(selected), len(revealed), result.predicted_cost, self._remaining_budget, policy))
        self._round_index += 1
        self._fit()
        # Labels are summarized by count only; the raw values never enter result.
    ...
    def finalize(self) -> dict[str, Any]:
        if self._finished:
            raise ProtocolViolation("final evaluation was already performed")
        self._finished = True
        oracle_ids = sorted(self._oracle.oracle_ids)
        if not oracle_ids:
            return {"status": "INCONCLUSIVE", "reason": "no oracle partition"}
        y_true = np.asarray([self._oracle.labels[idx] for idx in oracle_ids], dtype=int)
        y_prob = self._model.predict_proba(self._matrix(oracle_ids))
        metrics: dict[str, Any] = {"oracle_n": len(oracle_ids), "rounds": len(self.records), "revealed_labels": len(self._revealed)}
        if len(set(y_true)) > 1:
            metrics["roc_auc"] = float(roc_auc_score(y_true, y_prob))
            metrics["average_precision"] = float(average_precision_score(y_true, y_prob))
        else:
            metrics["status"] = "INCONCLUSIVE_SINGLE_CLASS_ORACLE"
